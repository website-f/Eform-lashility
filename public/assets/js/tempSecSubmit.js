...
==================================================================================================================================
*/
document.getElementById('tempsecsubmit-btn').addEventListener('click', saveTempSecSubmit);
...
function convertImageTempSecToBase64(File, label, fieldType, submitedForm) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.readAsDataURL(File);
    reader.onload = function () {
...
async function saveTempSecSubmit() {
    const submitedForm = [];
    const formType = document.querySelector('.formTypeTempSec').textContent;
    ...
    const signConversionPromises = [];
    const imageConversionPromises = [];

    fieldType.forEach((fieldsType) => {
       
        const labelName = fieldsType.querySelector('.form-labelTempSign');
        const label = labelName.textContent;
        const fieldVal = fieldsType.querySelector('.inputTypeTempSign');
          ...
          const fileInput = fieldsType.querySelector('input[name="file"]');
                ...
                if (fileInput && fileInput.files.length > 0) {
                  ...
                  imageConversionPromises.push(
                    convertImageTempSecToBase64(File,label, fieldType, submitedForm )
                  );
                } else {
   ...
   /* if (approval) {    
    ...
    let submit = [formType, submitedForm, formPublisher];
