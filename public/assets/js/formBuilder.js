...
const container = document.querySelector('.form-builder')
  ...
  draggers.forEach((hoverContainer) => {
    hoverContainer.addEventListener('mouseover', () => {
      hoverContainer.classList.add('hover-effect');
    });

    hoverContainer.addEventListener('mouseout', () => {
      hoverContainer.classList.remove('hover-effect');
    });
  });



draggers.forEach(draggable => {
...
// Function to handle dragging started
function drag(event) {
  ...
  function drop(event) {
    ...
    if (data == " Text Input") {
        ...
        fieldID.value = 'textInputLabel'+Date.now();
    ...
    label.setAttribute("for", idValueCheck)
    ...
    let checkboxes = document.querySelectorAll(".checkboxOptionBuild label");
   ...
   `;
    ...
    options.forEach((text) => {
      if (text.trim() !== '') {
        ...
        const newLabel = document.createElement("label");
        newLabel.classList.add("form-check-label");
        newLabel.setAttribute("for", idValueCheck)
        newLabel.textContent = text.trim();
    ...
    if (requiredField.checked) {
      const textInput = document.querySelector(`input[id=${idValueCheck}]`);
      textInput.required = true
     } 
  ...
  /*==========================================HEADING=============================================================================*/
    ...
    fieldIDValue.type = "fieldID"
    fieldIDValue.classList.add("formFieldHide");
    fieldIDValue.value = 'heading'+Date.now();
    
    inputDiv.appendChild(input)
    input.appendChild(fieldIDValue)
  ...
  /*==========================================IMAGE=============================================================================*/
    ...
    label.classList.add("labelBold")
    const idTime = Date.now() //set an unique id for each label
    const idValueImage = "imageInputLabel-"+idTime
